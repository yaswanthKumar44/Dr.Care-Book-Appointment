    ...
    <div class="container">
        <div class="edit-form">
            ...
            <form method="POST" action="update-doctor.jsp" onsubmit="return validateForm()">
                ...
                <input type="text" id="mobile" name="mobile" value="<%= rs.getString("MOBILE") %>">
                <label for="age">Age:</label>
                <input type="text" id="age" name="age" value="<%= rs.getString("AGE") %>">
                <label for="password">Password:</label>
                <input type="password" id="password" value="<%= rs.getString("password") %>" name="password" minlength="8" maxlength="14" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*\W)(?!.*\s).*$" title="Password must contain at least one uppercase letter, one lowercase letter, one number, one special character, and be between 8 and 14 characters long">
                <button type="submit">Update</button>
                <button type="button" onclick="deleteDoctor()" style="background-color: #ff0000; color: white;">Delete</button>
            ...
            function deleteDoctor() {
                if (confirm("Are you sure you want to delete this doctor?")) {
                    window.location.href = "delete-doctor.jsp?doctorId=<%= doctorId %>";
                }
            }
            </script>
