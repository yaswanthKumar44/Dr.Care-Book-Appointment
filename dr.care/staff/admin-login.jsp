...
try {
    ...
    try (Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE", "yash", "455664")) {
        String query = "SELECT fullname,email,address,age,gender,mobile,staff_type FROM admins WHERE username=? AND password=?";
        try (PreparedStatement pst = con.prepareStatement(query)) {
            ...
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
                    ...
                    session.setAttribute("staff_type", staffType);
                    session.setAttribute("username", usern);
                    session.setAttribute("password", pass);

                    response.sendRedirect("admin-profile.jsp"); // Redirect to profile page
                } else {
                    // User does not exist in the database, display login failed message
                    out.println("<script>alert('Login failed. Please check your username and password.'); window.location='admin-login.html';</script>");
                }
            }
        }
    }
